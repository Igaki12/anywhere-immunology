...
export const useTechnicalTerm = () => {
  const [technicalTerm, setTechnicalTerm] = useState([
    ...
    },
    {
      ...
      explanation:
        '抗原がレセプター（タンパク質・多糖などの高分子）と結合する部位。抗原の中の一部分だけが認識される。抗原決定基ともよばれる。タンパク質の一次構造に依存するものを線状エピトープ、タンパク質の立体構造に依存するものをコンフォメーショナルエピトープとよぶ。',
    },
    {
      term: [
        'MHC',
    ...
    },
    {
      ...
      explanation: 'CD4+CD8+のT細胞で、αβ型TCR発現している。正の選択がなされSPT細胞(Single Positive T cell)に成熟していく。幹細胞→DNT細胞→DPT細胞→SPT細胞（未熟T細胞）→ナイーブ成熟T細胞の順に分化していく。',
    },
    {
      term: ['DNT細胞', "Double Negative T cell", "プロT細胞", "プレT細胞"],
      explanation: 'CD4-CD8-のT細胞で、プロT細胞やプレT細胞に分類される。プロT細胞ではD-J再構成やV-DJ再構成などの遺伝子再構成が行われ、プレT細胞ではプレTCRの発現や対立遺伝子の排除（T細胞一つにつき1つの遺伝子を守るための機構）が行われている。T幹細胞→プロT細胞→プレT細胞→DPT細胞→SPT細胞→ナイーブ成熟T細胞の順に成熟していく。',
    },
    // {
    //   term:[''],
    //   explanation:'',
